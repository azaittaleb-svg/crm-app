...
interface ClientXlsxModalProps {
  isOpen: boolean;
  onClose: () => void;
  existingClients: any[];
  ownerId: string;
...
export const ClientXlsxModal: React.FC<ClientXlsxModalProps> = ({
  isOpen,
  onClose,
  existingClients,
  ownerId,
    ...
    const templateData = [
      {
        ...
        'Adresse Ligne 1': '123 Rue de la Liberté',
        'Adresse Ligne 2': 'Appt 4',
        Ville: 'Casablanca',
        ICE: '123456789012345',
        Notes: 'Client fidèle, grossiste',
      },
      {
        Nom: 'STE ALPHA SARL',
        Téléphone: '0522001122',
        Email: 'contact@alpha.ma',
        'Adresse Ligne 1': 'Zone Industrielle Oulad Saleh',
        'Adresse Ligne 2': '',
        Ville: 'Bouskoura',
        ICE: '987654321098765',
        Notes: 'Règlement à réception',
      },
    ];

    const ws = XLSX.utils.json_to_sheet(templateData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Clients Modèle');
  ...
  const processFile = (selectedFile: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Convert to array of arrays using header: 1 for maximum resilience
          ...
          for (const s of matchers.contains) {
            ...
            if (hNorm.includes(sNorm)) {
              if (field === 'lastName' && hNorm.includes('prenom')) {
                return 10; // Low score for first name (prenom) if checked as general last name
              }
              if (
                field === 'lastName' &&
                (hNorm.includes('tel') ||
                ...
                return 0; // Never map phone/mail/ice/id columns as client name
              }
              if (
        ...
        for (let i = headerRowIndex + 1; i < rawRows.length; i++) {
          ...
          // Combine notes/status/activities
        ...
        // Filter valid rows (must have a valid Name)
        const valid = normalized.filter((c) => c.name && c.name.length > 0);

        // Calculate potential duplicates (name checks against existing list)
  ...
  const handleStartImport = async () => {
    ...
    try {
      ...
      // Keep track of names imported in this same batch to prevent internal duplicates
      const importedInThisBatch = new Set<string>();

      for (const clientData of validRows) {
        const clientNameNorm = clientData.name.toLowerCase().trim();

        if (
          skipDuplicates &&
          (existingNamesLowercase.has(clientNameNorm) || importedInThisBatch.has(clientNameNorm))
        ) {
          skippedCount++;
          continue;
        }

        importedInThisBatch.add(clientNameNorm);
        ...
        batch.set(newClientRef, {
          ...
          createdAt: new Date(), // Local Date or Server timestamp
        });
      ...
      if (importedCount > 0) {
        await batch.commit();
        showToast(
          `${importedCount} client(s) importé(s) avec succès. ${skippedCount > 0 ? skippedCount + ' doublon(s) sauté(s).' : ''}`,
          'success'
        );
      } else {
        showToast('Aucun nouveau client importé (uniquement des doublons passés).', 'info');
      }
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          {/* Backdrop conforming to custom transparent styling */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
              ...
              <div className="p-6 space-y-5">
                ...
                {!file && (
                  <div className="space-y-4">
                    <p className="text-slate-500 dark:text-slate-400 text-[13px] leading-relaxed">
                      Téléchargez et préparez votre fichier client. Vous pouvez inclure le nom
                      complet, le téléphone, l'email, l'adresse, la ville, l'ICE, ainsi que vos
                      notes.
                    </p>
                ...
                {file && !isParsing && (
                  <div className="space-y-4">
                    ...
                    {/* Settings / Duplicates Action Toggle */}

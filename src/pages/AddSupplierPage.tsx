...
import { Truck, ArrowLeft, Phone, Mail, User, StickyNote, Globe } from 'lucide-react';
  ...
  const addSupplier = async (e: FormEvent) => {
    ...
    try {
      ...
      confirm({
        ...
        message: `Voulez-vous ajouter le fournisseur "${formData.name.trim()}" ?`,
        onConfirm: async () => {
          try {
            setLoading(true);
            await addDoc(collection(db, 'suppliers'), {
              ...
              address: formData.addressLine1 || null, // Keep compatible with single address field
      ...
      console.error('Outer add supplier error:', error);
      setLoading(false);
    }
  };
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-lg bg-[#696cff]/10 flex items-center justify-center">
                <Truck className="text-[#696cff]" size={20} />
              </div>
              <div>
            ...
            <div className="pt-6 border-t border-slate-200">
              ...
              >
                <Truck size={18} />
                {loading ? 'Enregistrement...' : "Finaliser l'enregistrement"}
              </button>
            </div>
          </motion.form>
          ...
          <div className="mt-12 text-center space-y-2">
            ...
            <p className="text-sm text-slate-600 leading-relaxed max-w-sm mx-auto font-medium italic">
              "Les données fournisseurs sont sécurisées et réservées exclusivement à l'usage interne
              de votre commerce."
            </p>
          </div>
        </div>

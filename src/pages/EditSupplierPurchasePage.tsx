...
export default function EditSupplierPurchasePage() {
  ...
  const [exchangeRate, setExchangeRate] = useState('9.4');

  // Trigger recalculation when isMotcho or exchangeRate changes
  useEffect(() => {
    if (isMotcho) {
  ...
  useEffect(() => {
    ...
    import('firebase/firestore').then(({ getDoc, doc }) => {
      getDoc(doc(db, 'suppliers', supplierId)).then((sSnap) => {
        if (sSnap.exists()) {
          setSupplierName(sSnap.data().name || 'Inconnu');
        }
      });
    });
    ...
    const unsubPurchase = onSnapshot(
      ...
      (snap) => {
        if (snap.exists()) {
          ...
          setIsMotcho(pData.isInternational || false);
          if (pData.exchangeRate) setExchangeRate(pData.exchangeRate.toString());
  ...
  const handlePasteExcel = () => {
    ...
    // Fallback if metadata wasn't detected
    ...
    const actualIsMotcho =
      isMotcho ||
      parsedSupplierName.toUpperCase().includes('MOTCHO') ||
      (supplierName && supplierName.toUpperCase().includes('MOTCHO'));
    ...
    if (newItems.length > 0) {
      ...
      if (actualIsMotcho || newItems.some((i) => i.prix_achat_usd && i.prix_achat_usd > 0)) {
        setIsMotcho(true);
      }
  ...
  const handleAiExtraction = async () => {
    ...
    setExtracting(true);
    try {
      const data = await backendService.extractItems(aiContent, parseFloat(exchangeRate) || 9.4);

      if (data.items && Array.isArray(data.items)) {
        const rate = parseFloat(exchangeRate) || 9.4;
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.writeFile(wb, `Achat_Stock_${purchaseId?.slice(0, 8)}.xlsx`);
    ...
    try {
      ...
      if (attachment) {
        ...
        try {
          ...
          finalAttachmentUrl = await toBase64(attachment);
      ...
      const computedStatus = totalPayments >= total ? 'paid' : 'credit';
      const pAmountPaid = paymentStatus === 'paid' ? total : totalPayments;
      ...
      await updateDoc(doc(db, 'suppliers', supplierId, 'purchases', purchaseId), {
        items: itemsWithTax,
        ...
        totalQteGlobal,
        dueDate: computedStatus === 'credit' && dueDate ? new Date(dueDate) : null,
      ...
      showToast('Achat stock mis à jour avec succès', 'success');
      navigate(`/supplier-purchase/${supplierId}/${purchaseId}`);
    } catch (error) {
  ...
  return (
    <div className="w-full">
      <form id="edit-supplier-purchase-form" onSubmit={handleSubmit} className="w-full">
              ...
              <div className="p-8 md:p-12 print:p-10 font-sans min-h-[500px] print:min-h-[1123px] flex flex-col relative bg-white dark:bg-[#2b2c40]">
                ...
                <div className="flex justify-between items-start border-b border-[#dbdade]/40 dark:border-[#434460]/40 pb-6 mb-6">
                  <div>
                    ...
                    <div className="text-[15px] font-medium text-[#696cff] dark:text-[#b1b4ff] mt-1 mb-4 flex items-center gap-2 font-mono">
                      <span className="text-[#a1acb8] dark:text-[#707194]">#</span>
                      {purchaseId?.toUpperCase().slice(0, 8)}
                    </div>
                  </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <input
                    disabled
                    value={supplierName}
                    className="w-full bg-[#f8f7fa] dark:bg-[#232333] border border-slate-205 outline-none font-medium text-slate-500 dark:text-[#707194] text-xs rounded-lg px-2.5 py-1.5 cursor-not-allowed"
                  />
                </div>
              </div>
                    ...
                    <div className="overflow-hidden flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-xs font-bold text-[#566a7f] dark:text-[#dbdade] truncate w-40">
                          {attachment ? attachment.name : 'Pièce jointe'}
                        </p>
                    ...
                    <div className="flex gap-1">
                      {existingAttachmentUrl && !attachment && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="p-1.5 text-[#696cff] hover:text-[#5f61e6] bg-white dark:bg-[#2b2c40] rounded shadow-sm"
                        >
                          <Download size={14} />
                        </a>
                      )}
